...
class StorageUtil {
  ...
  /// 获取文案生成模型偏好（数字人页AI生成文案用）
  static String getScriptModel() {
    return getString('script_model') ?? '自动选择';
  }

  /// 保存文案生成模型偏好
  static Future<bool> setScriptModel(String model) {
    return setString('script_model', model);
  }

  /// 获取数字人视频模型偏好
  static String getVideoModel() {
    return getString('video_model') ?? 'wan2.2-s2v';
  }

  /// 保存数字人视频模型偏好
  static Future<bool> setVideoModel(String model) {
    return setString('video_model', model);
  }

  /// 获取TTS音色模型偏好（CosyVoice系统音色用）
  static String getTtsVoiceModel() {
    return getString('tts_voice_model') ?? 'cosyvoice-v3-flash';
  }

  /// 保存TTS音色模型偏好
  static Future<bool> setTtsVoiceModel(String model) {
