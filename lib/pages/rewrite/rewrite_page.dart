...
class _RewritePageState extends State<RewritePage> {
  ...
  Future<void> _startRewrite() async {
    ...
    try {
      // 模拟改写过程（实际调用AiRewriteService）
      await Future.delayed(const Duration(seconds: 3));

      setState(() {
        _versions = [
          {
            'text': '这是第一种改写版本，根据选择的$_selectedMode模式生成。'
                '改写后的文案保持了原文的核心信息，同时调整了表达方式和句式结构。',
            'score': 87,
            'is_selected': true,
          },
          {
            'text': '这是第二种改写版本，创意度稍高。'
                '在保持语义的基础上，采用了更多样化的表达方式，适合不同风格的口播场景。',
            'score': 82,
            'is_selected': false,
          },
          {
            'text': '这是第三种改写版本，创意度最高。'
                '更加大胆地调整了叙述角度和表达方式，相似度最低，去重效果最好。',
            'score': 78,
            'is_selected': false,
          },
        ];
